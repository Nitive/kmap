...
func encodeSetupKeymapEvents(t *testing.T, events []inputEvent) *bytes.Buffer {
	t.Helper()
	buf := &bytes.Buffer{}
...
func TestSetupKeymapWaitForNextKeyPressSkipsNonPressEvents(t *testing.T) {
	events := []inputEvent{
		...
		{Type: evKey, Code: setupKeymapTestKeyA, Value: 0},         // key release
		{Type: evKey, Code: setupKeymapTestKeyA, Value: 2},         // key repeat
		{Type: evKey, Code: setupKeymapTestKeyLeftShift, Value: 1}, // first press should win
		{Type: evKey, Code: setupKeymapTestKeyBackspace, Value: 1}, // would be next
	}
